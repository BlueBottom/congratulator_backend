...
using Congratulator.DataAccess.Repositories;
...
builder.Services.AddDbContext<CongratulatorDbContext>(
    options =>
    {
        options.UseNpgsql(builder.Configuration.GetConnectionString(nameof(CongratulatorDbContext)));
    });
...
app.UseCors(x =>
{
    ...
    x.WithOrigins("http://localhost:8000");
